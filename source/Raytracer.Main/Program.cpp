...
int main(int, char**)
{
	...
	std::memset(img_buffer, 255, img_width * img_height * 4);
			...
			threadpool.EnqueueTask([&, i, ii]
				{
					...
					write_color(&img_buffer[(i * img_width + ii) * 3], pixel_color, samples_per_pixel);
				});
		}
	}
...
#pragma endregion
		...
		//g_mainRenderTargetView->GetResource(&pBackBuffer);
		//g_pd3dDevice->CreateTexture2D(&tex_description, &tex_resource, &texture);
		//g_pd3dDeviceContext->CopyResource(pBackBuffer, texture);
		//pBackBuffer->Release();
		//texture->Release();
